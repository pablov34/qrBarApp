...
@Component({
  selector: 'page-nuevopedidoorderqr',
  templateUrl: 'nuevopedidoorderqr.html'
})

export class NuevoPedidoOrderQrPage {
  ...
  options = {
    headers: {
      ...
      'Token':'',
      'UserId':'',
      'BarTableQrcode':'F9447637-E766-40C2-8A85-FE3C8A43FBD7'
    }
  };
  ...
  {
    console.log("NUEVO PEDIDO ORDER QR - ionViewDidLoad");
    ...
    (
      (opts) => {
        ...
        {
         this.loadProducts(); //el producto solo se carga con mesa 1 (PRUEBA EN LAB)
        }
        else
        {
          //siempre pide leer la mesa (PROD)
          this.scanbar();
        }
  ...
  {  
    ...
    // depending on if we want to leave this view
      ...
      {
        this.storage.set('continueordertype','OrderQr')
      ...
      (ProductsFromStorage) => {
        if(ProductsFromStorage != null)
        {
           ...
           this.sConfirmar = this.sum.toString();
            ...
            {
              //siempre pide leer la mesa y el prod (EN PROD)
              this.scanbar();
            }
        }
        else
 ...
 //cargar los productos del bar (con mesa)
 loadProducts()
 {
   this.productswithqty = []; //reset
  console.log("LOAD PRODUCTS: bar table qr code" + this.options.headers.BarTableQrcode); 
   this.barService.getProducts(this.options).then
   (
     (res) => 
     { 
       ...
       /*if(this.thisbar == true) 
       {
         this.toast.show('Mesa encontrada', '5000', 'center').subscribe(
           toast => {
             console.log(toast);
           }
         );
...
barscanoptions = {
  ...
  prompt : "Coloque codigo de barra en el area de escaneo", // Android
  disableSuccessBeep: true // iOS and Android
  }

  scanbar()
  {
    this.barcodeScanner.scan(this.barscanoptions).then
    (
      (barcodeData) => 
      {
        if(barcodeData.cancelled != true)
        {
            if(barcodeData.text.trim() != null && barcodeData.text.trim() != "")
            {
              this.options.headers.BarTableQrcode = barcodeData.text.trim();
              this.loadProducts();
              
            }
            else 
            {
              this.toast.show('Mesa no encontrada', '5000', 'center').subscribe(
                toast => {
                  console.log(toast);
                  this.bSendedOrder = true;           //reset
                  this.selectedScanedProducts = [];  //reset
      ...
      (err) => //error en el barcode scanner
      {
           /* this.toast.show('Error al ejecutar scan: ' + err, '5000', 'center').subscribe(
              toast => {
                console.log(toast);
              }
            );
  ...
  scanprods() {
    ...
    this.barcodeScanner.scan(this.scanoptions).then
    (     
      (barcodeData) => 
      {
        if(barcodeData.cancelled != true)
        {
          this.selectedProduct = this.products.find(product => product.Qrcode.toString() === barcodeData.text.trim());
          if(this.selectedProduct !== undefined) 
          {
            ...
            if(this.repeated != -1)//lo encuentra, debe añadir uno mas al prod ya existente
            {
              ...
              this.selectedScanedProducts[this.repeated].Price = this.selectedProduct.Price *
          ...
          else 
          {
            ...
            this.toast.show('Producto no disponible', '5000', 'center').subscribe(
              toast => {
                console.log(toast);
              }
            );
...
{ 
 ...
 "OrderDate":"",
 "BarTableQrcode":"",
 "OrderTypeId":"1"     //OrderTypeId = 1 es En Bar OrderTypeId = 2 es para retirar OrderTypeId = 3 es a domicilio
}
...
btnaddproduct()
{
  if(this.platform.is('core') || this.platform.is('mobileweb')) //en LAB
  {
        this.verpedido = false;// (PRUEBA EN LAB) listado de productos
  }
  else
  {
        this.scanprods();
