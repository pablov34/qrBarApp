...
@Component({
  selector: 'page-nuevopedidoorderwqr',
  templateUrl: 'nuevopedidoorderwqr.html'
})

export class NuevoPedidoOrderWQrPage {
  ...
  {
    console.log("NUEVO PEDIDO ORDER WQR - ionViewDidLoad");
  ...
  {  
    ...
    // depending on if we want to leave this view
      ...
      {
        this.storage.set('continueordertype','OrderWQr')
      ...
      (ProductsFromStorage) => {
        if(ProductsFromStorage != null)
        {
           ...
           this.sConfirmar = this.sum.toString();
            ...
            {
              //siempre pide leer la mesa, pero no productos (EN PROD)
              this.scanbar();
            }
        }
        else
    ...
    (
      (barcodeData) => 
      {
        if(barcodeData.cancelled != true)
        {
            ...
            {
              this.toast.show('Bar no encontrado', '5000', 'center').subscribe(
                toast => {
                  console.log(toast);
                  this.bSendedOrder = true;           //reset
                  this.selectedScanedProducts = [];  //reset
...
orderitemdata =
 ...
 btnaddproductwqr()
{
  //Añadir productos sin usar scaner
   //this.barservice.showLoading();
